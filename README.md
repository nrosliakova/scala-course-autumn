...
* It is required that the repository **must be named** as `scala-course-autumn` as the original one.
...
* Add collaborators: [Igor Wolkov](https://github.com/IgorWolkov);
...
For manually cloning create a new folder called `scala-course-autumn`, move to the folder and run
````shell
...
described in `.scalafmt.conf` file in the root (`scala-course-autumn`) folder.
...
8. Assign reviewers: [Igor Wolkov](https://github.com/IgorWolkov).
